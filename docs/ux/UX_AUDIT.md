...
- **The outer display's width is not settled.** ~411 dp (brief), ~443 dp at stock 390 dpi (adaptive study), ~524 dp at the owner's 330 dpi (device pass table; the adaptive study marks that value unverified). The JVM `fold-outer` captures use a provisional 411 dp at 420 dpi. At every one of these widths the loaded model id (≈ 420 dp of chip) still leaves the command field under ~40 dp (computed for 443 and 524 dp).
...
- **A partial answer after cancellation:** the chat audit says the assistant row is never written ([§4.10](audit/AUDIT_CHAT_MODELS_SETTINGS.md#410-in-flight-answers-are-lost-when-the-chat-leaves-composition-p0)); the adaptive study says the turn is persisted as interrupted. Unresolved; listed in [§10](#10-what-we-could-not-determine).
...
1. **The outer display's real width under the owner's density.** The device-pass table gives ~524 dp at 330 dpi; the adaptive study says the override may not apply to the outer panel; the JVM captures assume 411 dp. Settle with `adb shell wm density` and `dumpsys window displays` **while folded**. If the two panels differ in density, every fold also changes `densityDpi`, and `density` must join `configChanges` ([ADAPTIVE §1.4](research/ANDROID_ADAPTIVE_SAMPLES.md#14-one-measurement-still-owed-it-decides-a-manifest-flag)).
...
4. **What happens to a partial answer when its collector is cancelled.** The chat audit says it is never written; the adaptive study says it is persisted as interrupted. One device test settles it (and decides how bad UX-P0-07 is).
