...
**Caveat on gap-filling a reserved number (`skein-0m1z`).** `Migrator.migrate` applies only migrations whose version is strictly greater than the database's current `PRAGMA user_version`. A reserved number that lands *after* a higher one has already shipped — as 005 did, with 007 and 008 already in-tree — is therefore applied on a fresh install but silently SKIPPED on any database that already reached the higher `user_version`. Pre-v1 there are no shipped installs, so every real database is created fresh and gets 005. The same hazard applies to 002, 004 and 006 when they land. The general fix is an applied-migrations ledger (a `schema_migrations` table recording every version applied) rather than a single high-water mark; until that exists, a migration filling a reserved gap must say so in its header, as `005_export_stages.sql` does.
