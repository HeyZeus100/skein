...
**The vault integrity check's expected-object catalogue (`skein-hctx`).** `VaultLifecycle.integrityCheck()` compares the live `sqlite_master` against a catalogue it derives by replaying every migration `migrations/INDEX.txt` lists — matching each statement's `CREATE`/`DROP TABLE|INDEX|TRIGGER` the way SQLite itself applies them (including a `DROP TABLE` cascading to the indexes/triggers declared `ON` it, as 007 exercises for `attachment_keys`/`idx_attachment_keys_version`) — plus the `schema_migrations` ledger table above, which `Migrator` creates directly rather than via a numbered migration. Nothing here is hand-listed: a migration that creates or drops a schema object only ever needs a new `migrations/INDEX.txt` line and file, never a change to the catalogue-deriving code, so the catalogue cannot go stale the way it once did when 007 dropped two objects the check kept expecting forever.
