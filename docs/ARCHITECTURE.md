...
| `:core:inference` | Android library | `:app` | `api(:core:model)`, kotlinx-coroutines-core, kotlinx-serialization-json | `core/inference/build.gradle.kts` | E4 (models/thermal: `skein-st1r`, `E4.I9`) |
...
(22 modules total: `:app`, `:core:agent`, `:core:model`, `:core:ipc`,
`:core:vault`, `:core:security`, `:core:inference`, `:core:rag`,
...
`grep include settings.gradle.kts` lists all 22 in one `include(...)` block.)
...
- **`IsolationGuardPlugin`/`IsolationGuardTask`** (`checkIsolationGuards`, applied per-module via `id("app.skein.guard.isolation")`) — two independent checks depending on which allowlist a module's path matches: (a) `:core:model`, `:core:markdown`, `:core:agent`, `:testing` must never apply an Android Gradle plugin; (b) `:inference-service`/`:embedder-service` may declare only the project/external dependencies in `IsolationGuardTask.SERVICE_ALLOWLISTS`. Spec §2.6, plan §2.4.
