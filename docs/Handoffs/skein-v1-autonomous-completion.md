...
**Status:** Working document, updated 2026-09-22 (late evening, after Fold smoke #1) · code head `0643f11` on `origin/main` — eight merges landed that evening out of the first on-device smoke (§2.0, last bullet); this file's own revision is the commit after it
...
## 2. Current State (2026-09-22 late evening, main @ 0643f11)
...
- **Later the same night, seven more merges and two design documents** (code head `835bfd9`, docs head `6f53857`+): `997f694` skein-t9h2 (real launcher icon generated deterministically from the owner's artwork under `docs/branding/`); `08a8c82` skein-pnqo (NoteTab never passed a suggestion source, so the `[[` popup was never composed; wikilink tap routing was unwired — both fixed, with a Robolectric proof that the live preview does render); `9265792` skein-67ak (live force simulation with per-node drag in the graph); `3052d7d` skein-hacu (Compose's own Up/Down caret handling skips the `OffsetMapping` on shorter rendered lines — the editor now handles Up/Down itself); `c4b8e97` skein-jit3 + skein-l9oi (the editor painted opaque black on the dark background at 1.09:1 because `BasicTextField` never inherits the content colour — it has its own surface token now at 12:1/15:1; Settings › Appearance System/Light/Dark with `AppearancePrefs`; the graph overlay is not threaded yet, skein-7jc5); `835bfd9` skein-lds9 **closed for real**: the first red run with the diagnostics showed the injected `performClick()` never reaching the Try-again button on the runner (a lost synthetic touch, not a stall), so the class's button clicks use `performSemanticsAction(OnClick)`; `2a5e539` `docs/design/SKEIN_HUB.md` (skein-1m7v) and `6f53857` `docs/research/POCKETPAL_RECON.md` (skein-e8ly) — see §8. Owner decision recorded: **no personal name in identifiers** — the Hub permission is `app.skein.permission.MODEL_TRANSFER`, and skein-376c renames the 87 `us.aherrera.skein.*` files, the AIDL package and the `us.aherrera.skein.documents` authority to `app.skein.*` before the first release (dispatch it ALONE on a green head). Owner then confirmed the `[[` popup, Create, and link tap; the first node-drag merge could not engage on a real finger (the detector listened on the Main pointer pass, which the inner pan handler consumes first) and was redone on `PointerEventPass.Initial` at `814fd4d` with a two-node test. Still unconfirmed: that second drag pass, editor contrast, the Appearance toggle, Up/Down with the paired keyboard, backlinks on the second note.
...
1. **CI first.** `CI` was red on 4 of the 6 main pushes on the evening of 2026-09-22, every time on the same `MainActivityComposeTest` retry case, while every head passed locally; the `Reproducible build check` stayed green throughout. skein-lds9 is **reopened** with the diagnostics now landed: from `0d099b2` on, a failure prints the full assertion message (which wait, unlock state, whether a session existed, the semantics tree) and uploads the JUnit XML as the `unit-test-results` artifact. The diagnostics then caught it on run 35818302856: a lost synthetic touch, fixed at `835bfd9` with `performSemanticsAction(OnClick)`; CI has been green since (2a5e539 and after). Run `tools/loop/ci-status.sh --require-green --ignore Emulator` before every dispatch; the console diagnostics stay in place, so if that class ever times out again the message names the wait and the state.
   **Owner decisions pending before the Hub tracks become work** (bd epic skein-rkrq): apply the proposed spec §2 non-negotiable 11 and the §3.1 "Hub ships after v1" line (`docs/design/SKEIN_HUB.md` §11.2, verbatim); confirm Hub is post-v1; then file the design's H0–H16 breakdown (H0 amends skein-cyq — six changes noted on that bead — and H1/H2 fold into the ask path) and the recon's held B-9…B-18. Already filed from the recon and safe to dispatch behind the ask path: skein-f9zu (memory estimator), skein-brwf (Automatic runtime rules incl. mmap-off for Q4_0/IQ4_NL), skein-hewz (bounded GGUF pre-check), skein-wt92 (adversarial GGUF fixtures), skein-r8ah (import/load integrity assertions), skein-xth5 (16 KiB ELF alignment CI check — cheap, any time). skein-376c (identifier rename) runs alone, first, on the owner's go.
...
5. **Then, and only then:** a Fold smoke that exercises "ask" (successor to skein-94fh) and the formal M0 matrix (Track B, §9). Formal M0 waits until that path exists. Smoke #1 itself (skein-94fh) is **in progress, not finished**: steps 1, 2, 8 and 9 passed on 2026-09-22 and the fixes for steps 4–6 (autocomplete, link tap, node drag) plus contrast, theme toggle and arrow keys are installed but await the owner's confirmation; step 7's data-intact check is implied by the note surviving lock/unlock cycles but not explicitly recorded. Expect the walk to keep filing beads — every step so far did.
