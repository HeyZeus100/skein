...
- **2026-09-23 midday — goal epic `skein-gg11` set, wave 1 merged, wave 2 in flight, the emulator lane runs real tests for the first time.** The owner set the autonomous goal ("run the local model inside Skein"); the epic's description is the definition of done and the wave plan, its notes are plan revision 1 (the OfflineLLM escalations folded in) and the classification lists. Merged since the morning, in order: `07e0509` skein-o5f7 (OfflineLLM review, `research/upstream/offlinellm/`, ESCALATIONS first); `d54e6d6` skein-3yal (diagnostic sanitizer, salvaged); `1f9bc06` skein-gg11.1 (CPU-only device restriction + `n_outputs_max = 1` for generation contexts only — the cap also bounds embeddings, so it is conditional); `67b8ece` skein-80p + f0sy + xth5 (tiny SmolLM2-135M Q2_K test model fetched at test-build time, golden greedy + determinism tests, emulator-lane PATH fix, ELF alignment guard — which **fails**: both libraries are 4 KiB-aligned under NDK r27c, fix is skein-gg11.5, release-blocking); `311f9ba` skein-91yy + 0rkg (`inspect(in InspectRequest)` → `ModelInspection`, load and inspect share one verification path, spill decision closed, explicit `kvClear` per generate); `242726e` skein-p8rn (`schema_migrations` ledger, salvaged; fixed a latent hang in the JVM SQLite double); `159e28c` skein-6as (chat surface + `SendPipeline` + the turn-state seam; bead stays **open** until the wiring bead skein-whg8 merges). Coordinator fixes along the way: a build-file ktlint miss (`609ea95`, rule in bd memory `merge-verification-module-check`), nine instrumented test names D8 cannot dex (`a36da1b`), three migration comments that named the statement sentinel literally (`a445e4e`, `f7c240e` — eight test files split on the bare sequence, skein-8qif), the lane runs with a lock-screen PIN and `--continue`. **The emulator lane** (`gh workflow run emulator.yml --ref main`, dispatch/nightly only) reached the tests on its third run: 90 ran / 40 failed, then 68 / 32 — all pre-existing test debt, filed as skein-hctx (integrity catalogue still expects `attachment_keys`, P1, dispatched), skein-gg11.6 (lock-mid-generation on the emulator, P1, under the epic), skein-2hzi (int8 vector blob), skein-ycrn, skein-vn6m, skein-khk9 (biometric-enrolment tests must assume-skip). Watch the next run's report artefact (`gh run download <id> -n connected-test-reports`), not its status line. skein-g5pv closed: all 13 outage worktrees triaged, two salvaged, mzm5 confirmed discard (still open). **Never run `worktree-gc.sh --apply` while agents are live** — a fresh agent's clean worktree classifies as MERGED (bd memory `worktree-gc-live-agent-trap`); remove finished ones by name. Two pre-outage unmerged branches are skein-9ck6. Owner decisions recorded today: M0 is to be declared on the CLI matrix with the E-1 label (recommended, awaiting the owner's word on skein-5hr); the OfflineLLM control-token bug is low severity, a courtesy public issue is drafted in the session; node drag deferred (skein-xzsi, two weeks); chat "thinking" UX is a hybrid (orbs before the first token, streaming after) on skein-cmoe.
- **2026-09-23 afternoon — wave 2 merged, wave 3 in flight, and the emulator lane caught a P0 the JVM suite cannot see.** Merged after `159e28c`, in order: `167aeb2` skein-gg11.2 (`backendReport` diagnostic, redacted load-error lines, chat-template fallback); `2ac7924`+`1131000` skein-1uw + ktvz + 5oi-app (`LlamaCppEngine` in `core/inference/engine`, 72 JVM tests; two coordinator fixes: the fake service gained the new AIDL method, and the shared contract test's cancel case now measures real time — it timed out on `runTest`'s virtual clock under load); `3601d75` skein-gg11.5 (16 KiB max-page-size link flags in both CMake files, ELF guard blocking; CI hash `0ff02724…`); `c4ba11e` skein-cyq + r8ah + hewz (`ModelManager`, `ModelRegistry` contract in `core/model` + impl in `core/vault`, migration 009, bounded GGUF pre-check); `73ee4a8` skein-hctx (integrity catalogue matched raw chunk text incl. a comment illustrating a DROP — fixed; **nothing calls `integrityCheck()`**, skein-fegd); `c7e2844` skein-7yy2 (all three app key-lifecycle failures were test defects, production zeroization proven at every copy site); `01b4b0b` skein-gg11.6 (lock-mid-generation proven on the JVM; the emulator failures were tests racing **oneway** session pushes — the same race exists for the engine's first load after unlock, **skein-gg11.8**, P1, Opus, in flight); `d74be62` skein-x0ro + 2hzi (`replaceChunks` reused deleted rowids — production fix; cross-restart reuse is skein-l8nc); cherry-picks of skein-ycrn/azzj/khk9-vault; `d1301b6` **P0**: migration 009 re-added `context_length`, `companions`, `license_spdx` that 001 already creates, so **every vault open on real SQLite failed** (emulator run 35851243301) while every JVM test passed — `FakeSkeinSQLiteNative` executes no DDL; `MigrationColumnUniquenessTest` now replays the DDL text and fails on any repeat (bd memory `jvm-fake-driver-executes-no-ddl`: **never install a head on the Fold before its emulator run has been read**); `6daaf29` skein-vn6m (provider test moved into the app test APK — a Haiku attempt that deleted the test was rejected); `2e69cf3` skein-8qif. **Emulator lane status** (`gh workflow run emulator.yml --ref main`, then read the `connected-test-reports` artefact per module — an `assumeTrue` skip and an `@Ignore` are both recorded as failures, skein-i1dz): the last read run was on `c7e2844` before the P0 fix; the run on `2e69cf3` is the first that can pass the vault and app suites. Remaining known lane items: skein-gg11.9 (engine tests read `tiny.gguf` from the wrong context), skein-gg11.7 (x86 CPU features empty), khk9's inference half, the OpenSSL tarball transient (skein-qj5l, cache + retry, in flight). **Wave 3 in flight:** skein-whg8 (app wiring: `/chat`, `/import model`, `/models`, lock → unload), skein-gg11.8, skein-qj5l + i1dz, skein-gg11.9, skein-gg11.7. **Held:** skein-brwf (needs `LoadRequest.useMmap`, an additive contract change, after gg11.8), skein-f9zu (the estimator needs `n_layer`/head dims that `ModelInspection` does not carry yet — extend inspect first), skein-gg11.4, skein-gg11.3, skein-fegd, skein-l8nc, skein-9ck6. **Owner decision pending:** per-persona sampling overrides (skein-5oi note; recommendation: defer past v1, call sites pass `null`).
- **2026-09-23 ~11:30 PT — session limit hit; state for the next session.** The API session limit (resets **04:40 America/Los_Angeles**) terminated all four in-flight wave-3 agents mid-work. Their worktrees are dirty and **must be resumed, not re-dispatched** (SendMessage to the same agent id; never `worktree-gc.sh --apply` while they exist): skein-whg8 (Sonnet, `agent-abd581c7e0cd51dd7`, 12 changed files, the ask-path wiring), skein-gg11.8 (Opus, `agent-ae1486ae9040dbeae`, 10 files, the oneway session-push contract fix), skein-gg11.9 (Haiku, `agent-ad8f85aba45a89096`, 1 file), skein-gg11.7 (Haiku, `agent-a9a6226c29587e2f5`, 2 files). Merged and pushed just before: `03aedec` skein-qj5l + i1dz (OpenSSL tarball cached in all three lanes with one retry; `@Ignore` excluded from the connected run via `notAnnotation`). **Emulator lane on `2e69cf3`** (run 35853455054, the first past the migration P0): app 37 ran / 10 failed, vault 123 / 12, inference-service 28 / 2 — the vault opens again. What is left, all filed: **skein-gg11.10 (P1)** FTS5 `bm25()` fails "in the requested context" on device (search, DoD step 4) and a deleted document's revisions survive (cascade / per-connection `foreign_keys`); **skein-gg11.11 (P2)** `ModelRegistryImpl.default()` returns a `row-does-not-exist` sentinel instead of null; skein-juto (Migrator tests pin version 8), skein-v71i (ExportStage fixture loads too few migrations), skein-2rn4 (lock-policy test: activity STOPPED after recreate), gg11.9 / gg11.7 / khk9-inference (agents above), and `SchemaLoadInstrumentedTest` "statement #2: --" (see the next bullet or bead when diagnosed). CI and the two-runner reproducible build are green at `2e69cf3`. **Resumption order after the reset:** (1) resume the four agents; (2) merge whg8 → run the lane → merge gg11.8 → lane again; (3) dispatch skein-gg11.10 and gg11.11 (both block a truthful smoke); (4) only then `adb install -r` on the Fold and run skein-830f with the owner.
- **2026-09-23 morning (PT) — wave 3 merged; the ask path is wired; the Fold smoke is next.** After the session limit reset the four agents were resumed (not re-dispatched) and merged in order: `525e2ae` skein-gg11.8 (`onSessionUnlocked` is two-way so the engine knows the push landed; the two lock pushes stay `oneway` on purpose — a two-way lock push would let a wedged or compromised isolated process block the vault from locking; a second engine race fixed: the binding was published before the epoch re-send); `711999b` skein-gg11.7; `babb7c8` skein-gg11.9 (coordinator turned its early return into a hard failure — the lateinit fields would have been unset); `0a6c58c` skein-gg11.11 (test leakage, production correct); `51c8e56` skein-gg11.10 (**production**: `searchBodies` ran `bm25()` behind a window function in a CTE, which FTS5 cannot flatten; and `PRAGMA foreign_keys` ran only on keyed connections — the keyed production pool was already correct, the unkeyed test connection was not); `8b6ac79` **skein-whg8** (`app.skein.models.ModelServices` per session, `ManagedInferenceEngine` lazy-loads the registry default on first send and derives the chip/notifier status, TEARDOWN lock → `onSessionLocking` → guarded `unload` → `onSessionLocked` before the key zeroes, `/chat`, `/import model`, `/models`, five Robolectric tests; closes skein-6as). Coordinator fixes: a workflow-schema regression (`runner.temp` in job-level `env` made all three workflows invalid at `03aedec`; fixed at `811aa8c`), the emulator runner executing script lines separately (`4f1690e`). **Lane on `4f1690e`:** inference-service 28/0 (first fully green module), app 36/3, vault 123/11 — every vault item is fixed in `51c8e56` except skein-juto/v71i (test pins, Haiku in flight); app: skein-gg11.12 (two engine cases exceed the 60 s test budget on the emulator) + skein-2rn4 (Sonnet in flight). **Install rule:** the lane on `8b6ac79` (run 35882650886) must be read before `adb install -r`; the verified foss debug APK from that head is in `app/build/outputs/apk/foss/debug/`. Then skein-830f with the owner: `/new note` a fact → `/import model` → pick `/sdcard/skein-bench/models/Qwen2.5-3B-Instruct-abliterated-Q3_K_M.gguf` → `/chat` → ask → cancel mid-stream → lock → relaunch → ask again; record TTFT, tok/s, `backendReport`, `dumpsys meminfo :inference`, `/proc/<pid>/maps` for Vulkan/Mali mappings.
- **2026-09-23 09:30 PT — the emulator lane is green for the first time; the Fold smoke is the only open step of the goal.** `4ba091a` (run 35887417795): vault 123/0, inference-service 28/0, app 36 with one assumption skip (the death test cannot kill an isolated process from the test APK on the runner — its own bead; the JVM death test is the proof). Merged after `8b6ac79`: skein-gg11.12 + 2rn4 (engine contract budget overridable, 180 s in the instrumented subclass; lock-policy test seeds the notification-prompt flag), skein-juto + v71i (test pins), skein-ci54 (19 fixtures now seed parent rows — foreign keys stay enforced; zero production defects). skein-80p and skein-f0sy closed. The lane is now a real gate: run it on any head that touches migrations, the driver, the service or the engine, and read the per-module artefact. **Open:** skein-830f (the owner walks the six steps on the Fold with build `8b6ac79` installed — same app code as `4ba091a`; capture timings, `backendReport`, `dumpsys meminfo :inference`, `/proc/<pid>/maps`), then skein-7s1 (formal, needs the real models — skein-bxk, owner), skein-fc3 (M1 gate). Owner decisions still pending: per-persona sampling (skein-5oi), M0 labelling (skein-5hr), the OfflineLLM courtesy report, node drag (skein-xzsi, deferred), the icon file.
- **2026-09-23 ~15:20 PT — smoke #2 attempt 1 crashed at `/import model`; fixed at `43f5368`; the Fold now runs that build.** The owner's first walk of skein-830f died at step 3 three times with `IllegalArgumentException: Can only use lower 16 bits for requestCode` from `FragmentActivity.checkForValidRequestCode` (**skein-gg11.13**, P0): `MainActivity` extends `FragmentActivity` for `BiometricPrompt`, the only fragment artifact on the classpath was 1.2.5 (transitive from biometric 1.1.0), and that version still enforces the legacy request-code rule while `ActivityResultRegistry` issues codes ≥ 0x10000 — so **every** `rememberLauncherForActivityResult` hosted by the activity threw on launch (GGUF picker, chat and note attach pickers, vault key export, vault restore). Fix: `androidx.fragment:fragment` pinned to 1.9.1 in `:app` (catalog + `verification-metadata.xml`, additive) and a Robolectric case that submits `/import model` and asserts the `ACTION_OPEN_DOCUMENT` start — red-step verified: with the pin removed it fails with the device's exact exception. The "menu items including Settings stop working" report from the same walk is the crash cycle: after crash 2 the window was force-finished and no `app.skein` process existed for six minutes; the taps hit a dead surface. **Lane on `43f5368` (run 35923736113, attempt 1):** vault 123/0, inference-service 28/0, app 36/2 — the known w8bh assumption skip plus a new one, `LockPolicySettingsRecreationInstrumentedTest` ending STOPPED after `recreate()` (**skein-gg11.14**, P2): it passes locally in the lane's order (36/36 on an API 35 ATD), the lane uploads no logcat (that gap is step (b) on the bead), and attempt 2 decides deterministic vs flaky. CI and the reproducible build are green at `43f5368`. The install rule was honoured (lane read first); the Fold got the `43f5368` foss debug APK at 22:04Z (sha `d19fbb74…`, `.agent-logs/installed-apk-sha.txt`; rollback copy of the `8b6ac79` build in `.agent-logs/rollback-8b6ac79-foss-debug.apk`), and two forced recreations (`font_scale` 1.15 → 1.0) at the unlock gate returned to RESUMED with the same pid and zero exceptions — fold/unfold is safe under fragment 1.9.1. **Open:** the owner unlocks (biometric — nothing the runner can do) and walks skein-830f steps 1–6 on this build; the runner then verifies drawer → Settings and the picker with the uiautomator script noted on gg11.13. Housekeeping the classifier refused this session: 21 merged agent worktrees + 2 merged remote `claude/agent-*` branches are still present (`worktree-gc.sh --apply` when a human is at the keyboard).
- **2026-09-23 ~16:30 PT — the picker works; the import did not (fixed at `ae93bc0`); the lane's recreate failure is the lane's keyguard, not the app.** With `43f5368` on the Fold the owner reached the system picker (no crash) and the store logged a complete 1.59 GB copy at 15:46 — then `:inference` never started, the store directory vanished, and retries showed "Import failed: FromStore". **skein-gg11.15 (P0):** `GgufPreCheck` reads a 1 MiB prefix and refused `PREFIX_EXHAUSTED` inside the key/value walk; the smoke model's metadata ends at 5.66 MiB (`tokenizer.ggml.tokens` 2.6 MB, `merges` 2.7 MB — measured on the device's file), so **every real model was refused after the copy** and only the lane's tiny fixture ever passed; `finishGeneratedImport` then deleted the store directory. Second defect: the owner's retry was killed mid-copy (app swiped from Recents) and left `<id>/model.gguf.tmp`, which `ImmutableModelStore.import` refused as `AlreadyImported` on `directory.exists()`. Third: refusals were silent in logcat and the row showed a bare class name. Fixed at `ae93bc0`: exhausted-after-valid-header → `Plausible` (`:inference` inspection remains the authority), a `*.tmp`-only staging directory is cleared, `ImportRefusal.describe()` logged at W and shown in the row; red-step verified (both behaviour tests fail with the production change reverted), core:inference 300/0, app 372/0. The stale directory on the Fold was removed by hand (`run-as … rm -rf files/models/<id>`). **skein-gg11.14 (P2), diagnosed from the lane's own logcat (now uploaded as `lane-logcat/logcat.txt`, 64 MB ring):** all 25 `MainActivity` launches in a run are RESUMED for 3–24 ms then STOPPED with nothing else started; the keyguard goes GONE → LOCKSCREEN once and never leaves. First fix (`47f6415`, dismiss before PIN) did not help because the runner hands control over ~90 s **before** `BOOT_COMPLETED` posts, so SystemUI raised the lockscreen after the PIN existed. `ae93bc0` waits for the boot-complete properties, dismisses, sets the PIN, then polls for two minutes and types the PIN if the keyguard is up. If the recreate case still fails on that run, the keyguard is not the cause and the events buffer (`wm_pause_activity` reasons) is the next probe. Rules recorded: bd memory `emulator-lane-dismiss-keyguard-before-pin`; the lane uploads logcat now. **Open:** read the lane on `ae93bc0` (run 35932705384) → install the `ae93bc0` foss debug APK (built, sha `75445c81…`) → the owner re-runs `/import model`, then `/chat`; skein-w8bh (assumption skip recorded as failure) keeps the app module from ever reading fully green.
- **2026-09-23 ~16:50 PT — `ef2e0f2` on the Fold; the import runs off the main thread with a progress bar; the lane's recreate case passes.** Fourth cause on skein-gg11.15, found live: after the picker closed the Fold showed a black screen because `ModelManager.import` did both blocking passes on the collector's thread and `MainActivity` collects from a `rememberCoroutineScope` on Main (`/proc/<pid>/task/<pid>/stat` state R, minutes of CPU, no frames). Fixed at `ef2e0f2`: `ModelManager(io = Dispatchers.IO)` + `channelFlow(...).buffer(128).flowOn(io)`; a `ProgressReporter` counts bytes from the hash loop and a counting `InputStream` around the store's copy (1 % ticks, total = 2·size for a picked file); the status row shows "Importing model… N%" with a `LinearProgressIndicator` (tag `main_activity_import_progress_bar`). Tests pin the dispatcher ("import-io") and the tick contract (interior ticks in both passes over an 8 MiB fixture, the store reads 4 MiB chunks). **Lane on `ef2e0f2` (run 35933829722):** app 36/1 — the w8bh skip only, i.e. `LockPolicySettingsRecreationInstrumentedTest` **passes** (skein-gg11.14 closed: wait for the boot-complete properties, then dismiss, then PIN, then poll-and-type); inference-service 28/0; vault 123/1 — `PersonaServiceImplContractTest.observeAll_emits_after_create` timed out after 5 s of *virtual* time, a flake in an untouched module, filed P3. CI green at `ef2e0f2`. Installed at 23:48Z (sha `3bb55114…`); the old build's killed import left a `*.tmp` staging directory again, which the new store clears on the next import. **Open:** the owner unlocks, `/import model` → watches the bar → `/chat` → skein-830f steps 4–6; the runner then reads `.agent-logs/fold-smoke2-logcat-d.txt` for `ModelManager`/`ImmutableModelStore`/`:inference` lines and records TTFT, tok/s, `backendReport`, `dumpsys meminfo :inference`, `/proc/<pid>/maps`.
- **2026-09-23 ~20:00 PT — the copy landed; the registration did not (fixed at `ff6a3b6`, lane in flight); the owner's next unlock registers the copy that is already on the phone.** On `ef2e0f2` the import ran at ~1.7 MB/s (**skein-gg11.17**, P1: the store's copy pass hashes BLAKE3 in pure Kotlin; belongs in native/post-mmap), the status row was hidden under the keyboard (the command bar refocuses when the picker closes — "pops up then disappears"), and the owner queued several imports and swiped the app twice. The copy finished at 17:13:03 and the store sealed `model.gguf` (1,590,475,744 bytes); the import then waited on the sandboxed inspection, the vault's 5-minute idle lock tore down `UnlockedShell`, the `rememberCoroutineScope` that owned the import was cancelled, and the registry never got its row — `/models` read "No models imported yet" while a verified model sat in the store and a re-import was refused `AlreadyImported` (**skein-gg11.18**, P0). The inspection itself "took" 1.86 h: after the lock the isolated service stayed alive but frozen with the transaction outstanding (**skein-gg11.20**, P1, security-adjacent). `ff6a3b6`: `ImmutableModelStore.orphanedDirectories/adoptSealed` + `ModelManager.adoptOrphans` (SHA-256 re-hash only, same tail as a picked import, default when none, BLAKE3 left null = "no post-mmap expectation"), `finishGeneratedImport` keeps sealed files on `SESSION_LOCKED`/`BUSY`/dead-service inspection errors, `ModelServices.unlocked` runs the rescue after the epoch push and cancels it on lock, `MainActivity` bumps `/models` and reports it, refuses a concurrent `/import model`, `imePadding()` on the row, chip reads "importing N%". Tests: core:inference 305/0, app 372/0. **Still in the composition scope:** the import job itself (**skein-gg11.19**, P1: app-scoped job + foreground notification). **Answer recorded for the owner:** the chat input is the message bar at the bottom of the `/chat` tab, not the command bar; the chip says "no model" until the first send loads the default lazily.
- **2026-09-24 ~13:20 PT — the adoption ran on the Fold and was killed twice by the owner because it looked stuck; the verifier's pure-Kotlin BLAKE3 made the inspection a 26-minute job (interim fix at `a86fa13`, lane in flight).** `ff6a3b6` did what it should at unlock ("adopted a sealed model directory" at 13:10:12 and again 13:10:54, `:inference` started for the inspection each time); the owner swiped the app away after ~37 s twice; the third attempt (13:10:55) was left alone. Why so slow: `InferenceService.inspect` runs the full load gate (`pinAndVerify`), and `ModelVerifier` computed SHA-256 **and** BLAKE3 in the pre-mmap pass and BLAKE3 again over the mapping — the Kotlin BLAKE3 is ~2 MB/s on the device, ~13 min per pass (skein-gg11.17), so inspect ≈ 26 min and every load the same. **`a86fa13`:** `DigestAccumulator`/`bothDigests` take `withBlake3`; `verifyBeforeMmapDetailed(observeBlake3 = false)` is SHA-256 only; gate 2 uses the declared BLAKE3 when the row has one (imported rows: one slow pass instead of two, until native BLAKE3) and otherwise `verifyAfterMmapSha256` re-hashes the mapping against `expectedSha256` (adopted rows: seconds; the §2.4 in-place-write detection is unchanged — a second digest over the mapping after gate 1's read). Trade-off (algorithm diversity for gate 2) recorded in `VerifyBinding.kt`'s header; the native BLAKE3 stays on gg11.17 and also fixes the 13-minute import copy. Tests: verify 14/0, inference 308/0, inference-service 486/0, app 372/0. Note for anyone reading the device: `pidof app.skein:inference` finds nothing — the isolated process is named `app.skein:inference:app.skein.inference.service.InferenceService`; use `ps -A | grep inference`. Also seen: an SELinux denial for `/dev/mali0` in the isolated process (GPU inaccessible there — expected under the CPU-only restriction; record it in the backendReport reading).
- **2026-09-24 ~14:05 PT — registered in 4.6 s on `a86fa13`; the chat composer was there all along, under the status row (fixed at `aece751`, lane in flight).** With `a86fa13` installed and the vault unlocked: "adopted a sealed model directory" 13:51:30 → `:inference` up 13:51:31 → `model verified: both gates passed files=1 gate2=sha256` 13:51:33 → "adopted an orphaned model into the registry" 13:51:34. `/models` lists it as default. The owner then reported "no chat text bar" and "`/models` only has a delete button": uiautomator showed (a) the overlay-slot status row "Registered … and set as default" + Dismiss spanning `[0,965]–[2152,1114]`, i.e. exactly over the chat composer, which reappeared at `[690,999]–[1937,1098]` the moment Dismiss was tapped; (b) `ModelsScreen`'s Close button at y 64–102 under the status bar (frame 0–136) — untappable, and Back sends the app home. **skein-gg11.21** at `aece751`: success rows auto-clear after 6 s (failures and a running import persist), `ModelsScreen` pads by `safeDrawing`, the chip shows "<default name> · not loaded" before the first send instead of "no model". Follow-ups noted on the bead (inset the overlay slot once for all overlays; focus hand-off to the composer on `/chat`; a snackbar host anchored above the composer). **Open:** the owner's first send on the Fold (load ≈ SHA-256 twice over 1.6 GB, seconds; then streaming) — the runner is watching `.agent-logs/fold-smoke2-logcat-g.txt` for the load and the first token; then skein-830f steps 4–6 and the backendReport/meminfo/maps readings.
- **2026-09-25 ~12:15 PT — the first send failed by design order, not by the model (fixed at `b33df70`, lane in flight).** Owner: "hi" → "generation failed, retry"; logcat had `RetrievalServiceImpl … degrading to lexical + graph` and then nothing — no `:inference` start, no engine refusal. **skein-gg11.22 (P0):** `SendPipeline.send` assembles the prompt with `countTokens` → `syncCountTokens` → `ContextBudget.countTokens` → `LlamaCppEngine.count()`, which is `connection.get() ?: throw ModelNotLoaded()` — it needs the bound service — while `ManagedInferenceEngine` only loaded the default lazily inside `stream()`, which runs *after* assembly. Every first send therefore threw `ModelNotLoaded` in the counter (`withTimeoutOrNull` does not swallow it), `ChatViewModel` caught it as an `InferenceException` without logging, and retry failed identically. Fix: `ManagedInferenceEngine.warmUp()` (= `ensureLoaded`) is passed to `SendPipeline` as a trailing `warmUp` parameter and runs right after the user turn is persisted; the view model logs the failure kind. Tests: `SendPipelineTest` (every count happens after warm-up), `ManagedInferenceEngineTest` (load / no-op / `ModelNotLoaded`). Same commit: **Enter sends** in the composer (soft-keyboard newline commit = send, hardware Shift+Enter = newline, the `[[` popup keeps Enter). **skein-gg11.23 (P2):** the owner sees a note's Backlinks strip over the chat composer; in the runner's dumps (unfolded, split) the strip sits beside the composer, and `TabHost` composes only the active tab — needs the owner's posture/split/note-tab details. Operational: yesterday's `adb logcat` capture died overnight (phone asleep, adb reconnect does not resume it) — start a fresh capture at the beginning of every session; `pidof app.skein:inference` finds nothing (use `ps -A | grep inference`); log-tail monitors must end in a flushing stage (bd memory `monitor-pipelines-must-flush`).
- **2026-09-25 ~23:10 PT — the model LOADS on the Fold; prompt processing never finishes (skein-gg11.24, P0; instrumentation at `2d8d193`, lane in flight).** On `b33df70` the owner sent three times. 22:51:36 `:inference` started, 22:51:40 `model verified: both gates passed gate2=sha256` (2.3 s), 22:51:41 `model loaded files=1 ctx=16384` (3.7 s binder call), chip showed the model name — **DoD step 6's load is proven on device.** Then each generation sat in prefill until cancelled: the first two by the owner's ■ (`llama_decode ret=2`, `compute status 1` = the abort callback; the service reports CANCELLED, so the UI shows nothing — correct, not a bug), the third for 11 minutes with four threads at ~94 % on cpu4–6 (1.795 GHz of 2.6) and cpu7 (1.396 GHz of 3.1), RSS flat at 2.45 GB, thermal status 1, until the runner cancelled it from the ■ button. Ruled out: per-token decoding (the JNI decodes one `llama_decode` per 512-token batch, `n_outputs_max=1`), thermal pausing (CPU saturated), a huge system prompt (the seeded default persona has `systemPrompt = null`; the vault holds a handful of tiny notes → the "hi" prompt should be a few hundred tokens). The service logged nothing between "model loaded" and the abort — no prompt size, no batch time, no stop reason — so `2d8d193` adds numbers-only lines: `prefill start: n_in= batches= ctx=`, `prefill batch i/n: n= ms=`, `generation done: reason n_in n_out ttft elapsed rate` (service) and the same GenStats client-side. The content-logging gate also matches literal words (`prompt`, `tokens`, `chunk`) inside interpolated strings — use neutral field names. **Ground truth still missing:** no M0 result was kept on the phone (`/sdcard/skein-bench/results` is empty) and Termux `sshd` is not running (connection closed on 18022), so `tools/m0-benchmark/run.sh --smoke` cannot run until the owner starts it. **Next:** install `2d8d193`, one send, read the batch timings; then decide between a prompt/context cap for CPU, a quant with fast ARM kernels (Q4_0/Q4_K vs the smoke Q3_K_M), or a real defect in the decode path. Also fixed today: **skein-gg11.22** (first send died counting tokens before the lazy load — `warmUp` before assembly; Enter sends); filed **skein-gg11.23** (Backlinks strip over the chat composer — needs the owner's posture/split details).
- **2026-09-26 ~01:40 PT — skein-gg11.24 ROOT CAUSE: the debug APK shipped ggml/llama.cpp compiled at -O0 (fix + CI gate in the commit carrying this bullet; lane pending; install after the lane is read).** AGP configures debug variants with `CMAKE_BUILD_TYPE=Debug`; the NDK's Debug flags are `-g -fno-limit-debug-info` with **no `-O` at all**, and only the JNI shim had `target_compile_options(-O2)`. The 106-token `llama_decode` on `2d8d193` was live (it honoured the ■: `graph_compute … status 1`) and simply ~100× slow. Evidence: `inference-service/.cxx/Debug/…/arm64-v8a/compile_commands.json` — 64 of 66 llama.cpp/JNI translation units with no `-O` flag; the shipped `libskein_llama.so` (APK sha `aa0079b5…` = the local fossDebug) disassembles to 2.27 M instructions with 241 stack spills and 86 calls per 1000 and only 46 `fmla`; the rebuilt one: 1.09 M instructions, 44 spills per 1000, 1,331 `fmla`, APK `.so` 40.0 → 33.4 MB. Fix: `native/llama/CMakeLists.txt` §4a `add_compile_options("$<$<CONFIG:Debug>:-O2>")` before `add_subdirectory(llama.cpp)`; release = RelWithDebInfo (`-O2 -g -DNDEBUG`), untouched, so README §4's reproducible hashes do not move. Gate `tools/ci/native-opt-level.sh` (red on the old Debug databases, green on all six after the rebuild) runs in `ci.yml` right after `assembleFossDebug`. Local gates on the rebuilt APK: jni-symbols (25 `Java_`), elf-alignment, manifest-audit PASS, no-content-logging clean, `:inference-service:check` green. **Two findings alongside, filed not fixed:** (1) **no dotprod/i8mm kernels in any variant** — 0 `sdot/udot/smmla/ummla` in 2.27 M instructions; `GGML_CPU_ARM_ARCH` unset → NDK `armv8-a`; the Fold (Tensor G4) has `asimddp i8mm sve2 bf16`, Pixel 6/7 have dotprod+fp16 but no i8mm → proposal `armv8.2-a+dotprod+fp16` floor after the -O2 baseline is measured (skein-gg11.26, hashes move → two-cold-build CI). (2) **Vulkan can never initialise inside `:inference`**: logcat `avc: denied { read write } … name="mali0" scontext=u:r:isolated_app tcontext=gpu_device`; AOSP `isolated_app_all.te`: `neverallow { isolated_app_all -isolated_compute_app } gpu_device:chr_file { rw_file_perms execute }` — policy, not this device. The shipped default backend `vulkan` is dead on device, every run so far was CPU-only, the M0 Vulkan cell (pp512 6.49 t/s, `ngl 99`) was Termux = `untrusted_app`, and 27.8 MB of the 40 MB `.so` is SPIR-V that cannot run there (skein-gg11.27; owner decision, §2.4). Ground truth via `tools/m0-benchmark/run.sh --smoke` still needs Termux `sshd` on the Fold; the app's own `prefill batch … ms` / `generation done … rate=` lines are the interim measurement.
- **2026-09-26 ~02:00 PT — the model ANSWERS on the Fold (skein-gg11.24 closed; `8a78e08` installed, sha `fce66bb6…`).** Lane on `8a78e08` (run 36230117599): inference-service 28/0, app 36/1 (w8bh skip), core 123/1 (gg11.16 flake, noted; failed job rerun). CI + Reproducible build check green (release bytes unchanged, as claimed). On device, prompt "definition of skein?": `:inference` up 01:53:35, `model verified … gate2=sha256` + `model loaded ctx=16384` in 0.6 s (warm cache), `prefill start: n_in=162 batches=1`, **`prefill batch 1/1: n=162 ms=12345`** (13.1 t/s), then **`generation done: reason=LENGTH n_in=162 n_out=1024 ttft=12374ms elapsed=375024ms rate=2.73/s`**. Threading verified (process CPU 3.7× the long-lived compute thread; four workers at 90–117 % in a live sample; cpu4–6 2.6 GHz, cpu7 3.1 GHz, thermal 0) — so 13 t/s prefill / 2.7 t/s decode is the honest four-thread **-O2, baseline-NEON, no-dotprod** number; skein-gg11.26 is the next lever. The answer ran to the **1024-token cap, never EOS** → skein-gg11.28 (P1). Vocab checked from the GGUF header pulled off the Fold: `eos_token_id=151645 <|im_end|>` CONTROL, `<|endoftext|>` CONTROL, stock Qwen2.5 ChatML template — EOS detection is intact; the load-time `control-looking token 128247 '</s>'` warning is llama.cpp's by-name EOG list forcing an ordinary BPE token spelled `</s>` into the stop set (harmless for EOS). So the model did not emit `<|im_end|>` in 1024 tokens: look at sampling defaults/penalties and the assembled prompt, and get the owner's description of the text (coherent essay vs. loop vs. hallucinated turns) — the runner cannot read the screen (FLAG_SECURE, no content logging). Also unconfirmed by the runner: whether the 1024 tokens rendered live in the chat pane (the uiautomator dump showed the chat list only). DoD steps 4–6 (cancel mid-stream, lock, relaunch, ask again) are next once the owner confirms rendering.
- **2026-09-26 ~02:30 PT — the owner's screenshots: the answer rendered live, and it is chrome-as-text babble (skein-gg11.28; diagnostic in the commit carrying this bullet).** The answer opens by echoing the question as plain text ("definition of skein? is defined as a function in the pikehlpr package…"), stays lowercase, and loops one sentence pair to the 1024 cap. That is the signature of a chat model that never saw real ChatML control tokens. `ChatTemplating` has exactly that path by design (skein-0ztk): if a message's content cannot be located verbatim in the render, the *whole* render is tokenized with `parseSpecial=false` and the chrome goes in as text — silently. Ruled out by reading: the JNI string round trip (`utf8.h`, proper UTF-8 both ways), role wire names (`Role.USER("user")`), and the `contents` list (the same `readContent` strings go to both `render` and `segment`). Not ruled out: a content the template cannot reproduce verbatim (the persona is `""` and is skipped; the final user message = `PromptGuard` fenced block + `User: <query>`), or simply a Q3_K_M abliterated 3B being this bad. So the service now logs, per request, `prefill layout: chrome=S data=C n_chrome=X n_data=Y closed=B tmpl_fallback=B` (counts/flags only). **Next send on the Fold answers it**: `closed=true` → find why `segment` missed the content and fix it; `closed=false` with `n_chrome≈13` → the prompt is right and the model/quant/retrieved-context is the problem (try the Q4_K_M variant in `models.yaml`, and a question with no matching notes). The lane's second attempt on `8a78e08` failed on the same gg11.16 flake only.
- **2026-09-26 ~02:40 PT — the Fold's vault keys were destroyed by a lock-screen removal; vault reset by owner decision; `3010c35` installed (skein-gg11.29, P0).** Sequence from the device log: 02:20:08 vault unlocked fine; 02:20:19 the owner's send was cancelled by the runner's own `/chat` tap (runner stopped, will not touch the device while the owner drives); **02:22:32 `LockSettingsService: Changing lockscreen credential … newCredentialType=NONE`** (the owner removed the screen lock while re-doing biometrics), 02:24:57 PIN set again, fingerprint re-enrolled. Android Keystore deletes every `setUserAuthenticationRequired` key when the secure lock screen is removed — both `skein_master_bio_v1` and `skein_master_cred_v1` — so from 02:23:19 every launch failed *instantly, before any prompt* (`UnlockManager: unlock outcome: kind=Failed` 0.5 s after process start, bursts of five per 600 ms, no `BiometricService` request from app.skein) at `keystore.decryptCipher` (alias gone → not `KeyPermanentlyInvalidatedException` → `Failed("cipher init failed: …")`) and the screen showed the generic "Authentication failed. Try again." **with no way out**: the reset affordance is gated on `EnvelopeUnreadable` and the restore-from-export flow only exists on the setup screen. The master key is unrecoverable without a passphrase export (none existed). Owner chose "reset, keep the model": the runner deleted `VaultReset`'s set via `adb shell run-as` (`keys/key-envelope.v1`, `vault.db` + `-wal`/`-shm`, `attachments/`, `cache/staging_export`), left `files/models` (1.4 GB) and `datastore/`, installed `3010c35` (lane run 36232201683 green: inference-service 28/0, core 123/0, app 36/1 = w8bh skip) and relaunched → "Set up your vault". Fix scope on the bead: classify alias-missing as its own `UnlockResult` and route it to a restore-or-reset screen; log `kind=Failed class=<n>` (numbers only); keep the biometric→PIN fallback as a secondary item; document that removing the device lock destroys the vault keys and that the recovery export is the only insurance. Also learned: `adb exec-out screencap -d 4619827677550801152 -p` captures the inner panel with Skein in front (bd memory `fold-screencap`).
...
- **skein-gg11.27** (P1; re-opens the "Vulkan kept" half of skein-gtym) — Vulkan can never initialise inside the isolated `:inference` process: the Fold denies `gpu_device` to `isolated_app`, and AOSP's `neverallow { isolated_app_all -isolated_compute_app } gpu_device:chr_file` makes that policy on every device, not a GrapheneOS quirk. The shipped default backend `vulkan` is therefore dead on device and every run so far was CPU-only. Options in the bead; recommendation A: default `cpu`, drop `GGML_VULKAN` from the foss build (the 33 MB `.so` is mostly SPIR-V), keep the M0 Vulkan cell as a Termux reference number. Surfacing the *effective* backend in `backendReport`/the chip is small and should happen whichever way this goes.
- **skein-gg11.26** (P1, measured decision) — raise `GGML_CPU_ARM_ARCH` to an `armv8.2-a+dotprod+fp16` floor (safe on every GrapheneOS Pixel; i8mm is Tensor G3+ only) once the -O2 baseline is measured on the Fold; release bytes change, so `reproducible-build.yml` and README §4 hashes move with it.
...
0. **The goal is bd epic `skein-gg11`** ("run the local model inside Skein — 'ask' with a citation on the Fold"), set by the owner on 2026-09-23. Its description holds the definition of done and the four waves; its notes hold plan revision 1 (the OfflineLLM escalations folded in) and the owner's standing instruction to revise the plan when research lands. Read `bd show skein-gg11` before this list — where they differ, the epic wins. Waves 1 and 2 are **merged** (2026-09-23, §2.0's last two bullets): the contract (inspect, spill decision, KV clear), the native CPU-only fix and page-size flags, the diagnostic, the engine, the model manager and registry, the chat surface (skein-6as open until wiring), the CI fixtures, and the lane-found fixes. **Wave 3 in flight:** skein-whg8 (app wiring — its inputs are the notes on skein-whg8, skein-6as, skein-1uw, skein-cyq), skein-gg11.8 (oneway session-push race; may change the engine's session methods to suspend-until-applied — whg8 adapts at merge), skein-qj5l + i1dz (lane infra), skein-gg11.9 and gg11.7 (lane test fixes). **Before wave 4 (skein-830f, Fold smoke #2):** the emulator lane on the merged wiring head must be read (vault + app suites), gg11.8 must be merged (or the first load after unlock will race), and the Fold gets `adb install -r` only then. Held for capacity or prerequisites: skein-brwf, f9zu, gg11.4, gg11.3, fegd, l8nc, 9ck6, khk9's inference half. Merge protocol reminders that bit today: run the changed modules' own `check` (not only `:app:check`) before pushing; for CI-only branches, watch the run. The OfflineLLM review is on `main` at `07e0509` (`research/upstream/offlinellm/`, ESCALATIONS first): E-2 (a "CPU-only" load still initialises a Vulkan backend), E-3 (a ~300–500 MiB logits reserve per context) and E-4 (no KV clear per generate) are verified in the tree and being fixed in wave 1; E-1 (the M0 CPU cell is an upper bound on the APK, not a prediction) is the owner's M0-labelling decision on skein-5hr; eight matrix proposals (B-4, B-6, B-9…B-14) and the courtesy upstream report of OfflineLLM's control-token bug are held for the owner.
