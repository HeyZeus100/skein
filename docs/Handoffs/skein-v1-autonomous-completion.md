...
0. **The goal is bd epic `skein-gg11`** ("run the local model inside Skein — 'ask' with a citation on the Fold"), set by the owner on 2026-09-23. Its description holds the definition of done and the four waves; its notes hold plan revision 1 (the OfflineLLM escalations folded in) and the owner's standing instruction to revise the plan when research lands. Read `bd show skein-gg11` before this list — where they differ, the epic wins. Wave 1 (dispatched 2026-09-23): skein-91yy + 0rkg (contract, Opus), skein-6as (chat surface against fakes, Sonnet), skein-80p + f0sy + xth5 (CI fixtures, Sonnet), skein-gg11.1 (native CPU-only devices restriction + `n_outputs_max = 1`, Sonnet). Wave 2 after the contract and native beads merge: skein-1uw + ktvz + 5oi (Opus) ‖ skein-cyq + r8ah + hewz (Sonnet) ‖ skein-gg11.2 (backendReport diagnostic) and skein-gg11.4. Wave 3: skein-whg8 (app wiring) ‖ skein-brwf + f9zu. Wave 4: skein-830f (Fold smoke #2, hardware runner). The OfflineLLM review is on `main` at `07e0509` (`research/upstream/offlinellm/`, ESCALATIONS first): E-2 (a "CPU-only" load still initialises a Vulkan backend), E-3 (a ~300–500 MiB logits reserve per context) and E-4 (no KV clear per generate) are verified in the tree and being fixed in wave 1; E-1 (the M0 CPU cell is an upper bound on the APK, not a prediction) is the owner's M0-labelling decision on skein-5hr; eight matrix proposals (B-4, B-6, B-9…B-14) and the courtesy upstream report of OfflineLLM's control-token bug are held for the owner.
