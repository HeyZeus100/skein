...
0. **The goal is bd epic `skein-gg11`** ("run the local model inside Skein — 'ask' with a citation on the Fold"), set by the owner on 2026-09-23. Its description holds the definition of done and the four waves; its notes hold plan revision 1 (the OfflineLLM escalations folded in) and the owner's standing instruction to revise the plan when research lands. Read `bd show skein-gg11` before this list — where they differ, the epic wins. Wave 1 is **merged** (2026-09-23 midday, §2.0): skein-91yy + 0rkg, skein-6as (open until wiring), skein-80p + f0sy + xth5 (80p/f0sy open until the emulator lane is green), skein-gg11.1. Wave 2 is **in flight**: skein-1uw + ktvz + 5oi (Opus, `core/inference` engine), skein-cyq + r8ah + hewz (Sonnet, registry in `core/model`+`core/vault`, manager in `core/inference/models`, migration 009), skein-gg11.2 (backendReport + redacted load errors, adds one JNI symbol) — plus skein-hctx (vault integrity catalogue). Still to dispatch in wave 2 after those merge: skein-gg11.5 (16 KiB page-size link flags, release-blocking), skein-gg11.4 (containment tests), skein-gg11.6 (lock-mid-generation on the emulator), skein-gg11.3 once the lane is green. Wave 3: skein-whg8 (app wiring: `/chat`, `/import model`, lock → unload; its injection list is on skein-6as's last note) ‖ skein-brwf + f9zu. Wave 4: skein-830f (Fold smoke #2, hardware runner). Merge protocol reminders that bit today: run the changed modules' own `check` (not only `:app:check`) before pushing; for CI-only branches, watch the run. The OfflineLLM review is on `main` at `07e0509` (`research/upstream/offlinellm/`, ESCALATIONS first): E-2 (a "CPU-only" load still initialises a Vulkan backend), E-3 (a ~300–500 MiB logits reserve per context) and E-4 (no KV clear per generate) are verified in the tree and being fixed in wave 1; E-1 (the M0 CPU cell is an upper bound on the APK, not a prediction) is the owner's M0-labelling decision on skein-5hr; eight matrix proposals (B-4, B-6, B-9…B-14) and the courtesy upstream report of OfflineLLM's control-token bug are held for the owner.
