...
1. **CI on the pushed head is confirmed green** (`cd9d1b3`, §2.0 — `CI` needed a rerun). Still run `tools/loop/ci-status.sh --require-green --ignore Emulator` before every dispatch. **First dispatch: skein-lds9** (Sonnet, the intermittent `MainActivityComposeTest` stall; read its NOTES — the diagnosis and repro recipe are there). Nothing else goes out until that test is deterministic or quarantined with a bead, because the §4.9 gate cannot distinguish a real regression from this flake.
