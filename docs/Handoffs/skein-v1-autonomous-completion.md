...
1. **CI first.** `CI` was red on 4 of the 6 main pushes on the evening of 2026-09-22, every time on the same `MainActivityComposeTest` retry case, while every head passed locally; the `Reproducible build check` stayed green throughout. skein-lds9 is **reopened** with the diagnostics now landed: from `0d099b2` on, a failure prints the full assertion message (which wait, unlock state, whether a session existed, the semantics tree) and uploads the JUnit XML as the `unit-test-results` artifact. Read the next red run's output before touching the test again; the standing hypothesis is a Compose-side stall under Robolectric (a `StateFlow` flip from `Dispatchers.Default` not producing the recomposition `waitUntil` polls for), and the next candidate fix is to run the test application's scope on the Compose test dispatcher. Run `tools/loop/ci-status.sh --require-green --ignore Emulator` before every dispatch; while this flake is open, a red `CI` whose only failure is that case does not block a wave, anything else does.
...
5. **Then, and only then:** a Fold smoke that exercises "ask" (successor to skein-94fh) and the formal M0 matrix (Track B, §9). Formal M0 waits until that path exists. Smoke #1 itself (skein-94fh) is **in progress, not finished**: steps 1, 2 (creation), 8 and 9 passed on 2026-09-22; finish steps 4–7 with the owner before closing it, and expect it to keep filing beads — every step so far did.
