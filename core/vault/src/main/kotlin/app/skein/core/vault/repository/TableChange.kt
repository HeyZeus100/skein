...
// a future `SearchViewModel`-style consumer later, mirroring the reserved
// slot called out in `IndexStoreImpl`'s header) can filter to only the
// changes they care about instead of re-querying on every write anywhere
// in the vault.
//
...
// tables (`E2.I15`) — that class reserves its own `ChangeBus` slot (see
// its header) rather than sharing this one.
